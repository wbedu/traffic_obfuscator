...
def store_data(pkt):
    timestamp = time()
    LOG.info(
        f"{timestamp},{pkt.getlayer(IP).src},{pkt.getlayer(IP).sport},{pkt.getlayer(IP).dst},{pkt.getlayer(IP).dport}"
    )
...
if __name__ == "__main__":
    ...
    sniff(
        ...
        filter="udp port 80 or tcp port 80 or udp port 443 or tcp port 443",
    )
